...
module.exports = {
  ...
  verifySource: function(url_req, callback) {
    ...
    if (config.allowExternal) {
       ...
       */
      if (config.localHostAllowed.indexOf(url.parse(url_req).host) >= 0) {
  ...
  verifyRemote: function(url_req, callback) {
    ...
    var req = http.request(options, function (r) {
      ...
      // exists
      callback(r.statusCode == 200 || r.statusCode == 301, url_req);
  ...
  verifyLocal: function(url_req, callback) {
     ...
     */
    if (url_req.indexOf('https://') >= 0)
      url_req = url_req.replace('https://www.', '/var/www/');
    else
